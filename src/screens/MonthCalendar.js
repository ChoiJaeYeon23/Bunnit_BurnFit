...
const MonthCalendar = () => {
    ...
    const [currentDate, setCurrentDate] = useAtom(currentDateAtom);      // currentDate를 Zotai로 관리
    const [selectedDate, setSelectedDate] = useAtom(selectedDateAtom);      // selectedDate를 Zotai로 관리
    ...
    const renderDay = ({ item }) => {
        ...
        return (
            <TouchableOpacity
                ...
                onPress={() => handleDayPress(item)} // 날짜 클릭 시 해당 날짜를 selectedDate로 설정
            >
                <Text
                    style={[ 
                        styles.dayText, 
                        isToday && styles.boldText, // 오늘 날짜(일자) 글씨를 진하게 
                        isOutsideCurrentMonth && styles.grayText, // 현재 달이 아닌 날짜(일자)는 회색 
                    ]}
                >
